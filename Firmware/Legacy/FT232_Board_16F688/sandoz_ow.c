// This code has been fiddled with quite a bit by me,
// If things don't work for you download the orginal library
// and work from that!
//ED
...
short int onewire_init_with_error_check() {
   ...
   output_float(PIN_A2); // float 1-wire high
...
void onewire_sendbyte(int data) {
   ...
   for (count=0; count<8; ++count) {
      ...
      output_bit(PIN_A2, shift_right(&data,1,0)); // set output bit on 1-wire
      delay_us( 60 ); // wait until end of write slot.
      output_float(PIN_A2); // set 1-wire high again,
...
int onewire_readbyte() {
   ...
   for (count=0; count<8; ++count) {
      ...
      output_float(PIN_A2); // now let 1-wire float high,
      delay_us( 8 ); // let device state stabilise,
      shift_right(&data,1,input(PIN_A2)); // and load result.
